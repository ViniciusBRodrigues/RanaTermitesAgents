...
isbX = Shared.getNumber("isBornPositionX")
isbY = Shared.getNumber("isBornPositionY")
last_position = {["x"]=isbX,["y"]=isbY}
...
function InitializeAgent()
	...
	say("Agent inquiline soldier #: " .. ID .. " has been initialized")
	
	Agent.changeColor{r=241, g=213, b=116}

	Speed = Shared.getNumber("inq_sol_speed")
	
	repulsionRange = Shared.getNumber("inquiline_repulsion_range")
	detectionRange = Shared.getNumber("inquiline_sol_detection_range")
...
function HandleEvent(event)
	...
	if event.description == "host_worker" 
		...
		then
			...
			Event.emit{description="interaction",table={first="is", second="hw"}}
			...
			--------------
				...
				say("Termite inquiline soldier #: " .. ID .. " was poisoned by the host worker!")
		
	if event.description == "host_soldier" 
		and Physics.calcDistance{x1=event.X, x2=PositionX, y1=event.Y, y2=PositionY} < detectionRange 
		then
		
			Event.emit{description="interaction",table={first="is", second="hs"}}
			...
			--------------
				...
				timeToLive = timeToLive - Stat.randomMean(2.8,9.54)*collision_callfrequency; -- losing life time
				Move.to{x= new_X, y=new_Y}
					
				say("Termite inquiline soldier #: " .. ID .. " was poisoned by the host soldier!")
				
	elseif event.description == "inquiline_worker"
		and Physics.calcDistance{x1=event.X, x2=PositionX, y1=event.Y, y2=PositionY} < detectionRange 
		then
			
			Event.emit{description="interaction",table={first="is", second="iw"}}

			Speed = Stat.randomMean(0.2,2)
		
			timeToLive = timeToLive + Stat.randomMean(0.47,4.77)*collision_callfrequency;

			say("Termite inquiline soldier #: " .. ID .. " was cured by the iquiline worker!")

		if timeToLive >= timeToLiveMax then timeToLive = timeToLiveMax end -- winning life time
		
	elseif event.description == "inquiline_soldier"
		and Physics.calcDistance{x1=event.X, x2=PositionX, y1=event.Y, y2=PositionY} < detectionRange 
		then
		
			Event.emit{description="interaction",table={first="is", second="is"}}

			Speed = Stat.randomMean(0.2,2)
		
			timeToLive = timeToLive + Stat.randomMean(1.4,4.77)*collision_callfrequency;  -- winning life time

			say("Termite inquiline soldier #: " .. ID .. " was cured by the iquiline soldier!")
...
function TakeStep()
	...
	if call_counter >= collision_callfrequency*1/STEP_RESOLUTION then
		Event.emit{description="inquiline_soldier"}
		call_counter = 0
	end
	...
	-- inquiline soldier lifetime
	--------------
	if timeToLive <= 0 then
	 	say("Inquiline soldier agent #" .. ID .." died at the old age of  ".. Core.time())
	 	Event.emit{description="inquiline_soldier_died"}
        Event.emit{description="isdistance",table={id=ID,dist=distance}}
...
function CleanUp()
	...
	say("Inquiline soldier agent " ..ID.. " distance= " ..distance)
